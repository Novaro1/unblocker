...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  // ── AI Monitor setup modal ──
  if (interaction.customId === "veil_ai_monitor_modal") {
    if (!isAdmin(interaction.member)) {
      return interaction.reply({ content: "Administrator permission required.", ephemeral: true });
    }
    const apiKey     = interaction.fields.getTextInputValue("ai_api_key").trim();
    const channelId  = interaction.fields.getTextInputValue("ai_channel_id").trim();
    const intervalRaw = interaction.fields.getTextInputValue("ai_interval").trim();
    const interval   = parseInt(intervalRaw);

    if (isNaN(interval) || interval < 5) {
      return interaction.reply({ content: "Interval must be a number of minutes (minimum 5).", ephemeral: true });
    }

    const channel = interaction.guild.channels.cache.get(channelId);
    if (!channel) {
      return interaction.reply({ content: "Channel not found. Make sure you copied the channel ID correctly.", ephemeral: true });
    }
    ...
    try {
      ...
      return interaction.reply({ content: `❌ API key validation failed: ${err.message}`, ephemeral: true });
    }

    setConfig(interaction.guild.id, {
      aiApiKey:            apiKey,
      aiAlertChannelId:    channelId,
      aiIntervalMinutes:   interval,
    });

    startMonitor(interaction.guild, interval);

    return interaction.reply({
      content: `✅ AI monitor active!\n\n• **Alert channel:** ${channel}\n• **Scan interval:** every ${interval} minutes\n• First scan in ${interval} minutes, or run \`/ai-scan\` now.`,
      ephemeral: true,
    });
  }
